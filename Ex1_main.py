...
def main():
    try:
        ...
        Reader_writer.read_calculate_write(buildings[1], calls_cases[0], f'{output}_B1_a')
        Reader_writer.read_calculate_write(buildings[0], calls_cases[0], f'{output}_B2_a')
        ind = 3
        for i in range(2, 5):
            ch = chr(97)
            ind1 = 1
            for j in calls_cases:
                st = output+"_B" + str(ind) + "_" + ch
